...
class SeniorCirqleHomeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_senior_cirqle_home)
        init()
    ...
    fun loaddata(){
        ...
        val collegeName= Utility.getUserDetails(this@SeniorCirqleHomeActivity)?.collegeName!!
        val reqCall= BuilderRetrofit.builService(ApiInterface::class.java).get_doubts(collegeName,filter,"doubtPost")
        reqCall.enqueue(object :Callback<GetDoubtsResponseModel>{
            override fun onResponse(call: Call<GetDoubtsResponseModel>, response: Response<GetDoubtsResponseModel>) {
                ...
                postAdapter= AskDoubtsPostAdapter(this@SeniorCirqleHomeActivity,response.body()?.post!!)
                home_rv.layoutManager=LinearLayoutManager(this@SeniorCirqleHomeActivity,LinearLayoutManager.VERTICAL,false)
                home_rv.adapter=postAdapter
            }

            override fun onFailure(call: Call<GetDoubtsResponseModel>, t: Throwable) {
                Toast.makeText(this@SeniorCirqleHomeActivity, "failed", Toast.LENGTH_SHORT).show()
